...
/** This class watches the Canvas for events. This class watches the Canvas for events. */
public class CanvasListener
    implements MouseListener, KeyListener, MouseWheelListener, MouseMotionListener {

  /** The Window. the Level window being watched. */
  private Level window;

  /** The Mouse's X position. The mouse's X Position. */
  private int mx = 0;
  /** The Mouse's Y position. The mouse's Y Position. */
  private int my = 0;

  /** The currently selected ability. The currently selected ability. */
  ...
  */
  ...
  /**
   * The abilities available for this level, and their count. The abilities available for this
   * level, and their count. -1 == no uses for this level.
   */
  private Stack abilLink;

  /** the remaining uses of the digger ability. the remaining uses of the digger ability. */
  private int digger;

  /** the remaining uses of the blocker ability. the remaining uses of the blocker ability. */
  private int blocker;

  /** the remaining uses of the floater ability. the remaining uses of the floater ability. */
  private int floater;

  /** the remaining uses of the exploder ability. the remaining uses of the exploder ability. */
  private int exploder;

  /** the remaining uses of the bridger ability. the remaining uses of the bridger ability. */
  private int bridger;

  /** the remaining uses of the climber ability. the remaining uses of the climber ability. */
  private int climber;

  /** the remaining uses of the miner ability. the remaining uses of the miner ability. */
  private int miner;

  /** the remaining uses of the basher ability. the remaining uses of the basher ability. */
  private int basher;

  /**
   * The Constructor, sets up the window to listen to. The Constructor, sets up the window to listen
   * to.
   *
  ...
  /**
   * This function watches for mouse clicks. This function watches for mouse clicks, and does an
   * appropriate action based on the position of the click.
   *
    ...
    if (yPos < 35) {
      ...
      for (int i = 0; i < lemmings.size(); i++) {
        ...
        if (lemmingSelected(
            ...
            yPos,
            (int)
                (window.getScale() * selectedLemming.getXPos()
                    + window.getScale() * window.getXOffset()),
            (int)
                (window.getScale() * selectedLemming.getYPos()
          ...
          switch (selectedAbility) {
            ...
            case 'e': // exploder
              if (exploder > 0 && !selectedLemming.isExploder()) {
                ...
                selectedLemming.setExpTime((int) HiResTimer.getInstance().currentTimeMillis());
                exploder--;
                abilLink.remove(4);
  ...
  /**
   * This function is only present to make Mouse Listener happy. This function is only present to
  ...
  /**
   * This function is only present to make Key Listener happy. This function is only present to make
   * Key Listener happy.
   *
  ...
  /**
   * This function does events based on key presses. This function does events based on key presses.
   *
   * @param e the key Event.
   */
  ...
  /**
   * This function scrolls the window based on scrolling the mouse. This function scrolls the window
   * based on scrolling the mouse.
   *
  ...
  /**
   * This function updates the mouse position. This function updates the mouse position.
   *
   * @param e the mouse Event.
   */
  ...
  /**
   * This function determines if the selected lemming. This function determines if you are on the
   * selected lemming.
   *
  ...
  /**
   * This function returns the currently selected ability. This function returns the currently
   * selected ability.
   *
  ...
  /**
   * This function figures out if you have selected the given lemming. This function figures out if
   * you have selected the given lemming.
   *
