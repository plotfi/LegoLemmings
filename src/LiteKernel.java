...
public class LiteKernel {
  ...
  public void move(Lemming lemming) {
    ...
    if (lemming.getFallCounter() > lemming.dropLen() / 5 && !lemming.isFloater()) {
      lemming.setFallDying(true);
    }
    if (lemming.getFallDying()
        && (Color.black).getRGB() != lemming.getBottomColor(0)
        && !(lemming.isFloater())) {
    ...
    if (lemming.isBridger() && !((Color.black).getRGB() == lemming.getBottomColor(0))) {
      lemming.bridge();
    }
    if ((((lemming.isClimber() && lemming.climbable(lemming.getRightDir()))
    ...
    if ((Color.black).getRGB() == lemming.getBottomColor(0)) {
      ...
      if ((Color.black).getRGB() == lemming.getBottomColor(2) && !(lemming.isFloater())) {
        yPos += 4;
        lemming.setDigger(false);
